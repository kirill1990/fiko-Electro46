...
public class ToExcelKOyear
{
	...
	public ToExcelKOyear(String year)
	{
		...
		@SuppressWarnings("rawtypes") Vector<Vector> values = new ConnectionBD().getDataFromDB_Year(year);
		...
		try
		{
			...
			// создание книги
			workbook = Workbook.createWorkbook(new File("Сетевые организации, список за " + year + "(" + dt + ").xls"), ws);
			// создание листа
			...
			sheet.addCell(new Label(0, 0, "Организация", cellFormat));
